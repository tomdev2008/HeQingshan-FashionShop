...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.loadFooterView = [[LoadingMoreFooterView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, LOADINGVIEW_HEIGHT)];
        self.loadingmore = NO;
...
- (void)cellSelected:(NSNotification *)notification
{
    if ([self.flowdelegate respondsToSelector:@selector(flowView:didSelectRow:)])
    {
        [self.flowdelegate flowView:self didSelectRow:(WaterFlowCell*)notification.object];
    }
}
...
{    
    ...
    scrollHeight=scrollHeight>self.frame.size.height?scrollHeight:self.frame.size.height;
    self.contentSize = CGSizeMake(self.frame.size.width, scrollHeight + LOADINGVIEW_HEIGHT);
...
- (void)pageScroll
{
    ...
    for (int i = 0 ; i< numberOfColumns; i++)
    {
        ...
        NSLog(@"cell:%d, x:%f, y:%f",cell.indexPath.row,cell.frame.origin.x,cell.frame.origin.y);
